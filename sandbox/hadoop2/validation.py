...
from sahara.plugins.sandbox.hadoop2 import config_helper as cu
...
def validate_cluster_creating(pctx, cluster):
    ...
    if spark_hist_count > 1:
        raise ex.InvalidComponentCountException('spark history server',
                                                ...
                                                spark_hist_count)
    # newly added
    sp_master_count = _get_inst_count(cluster, 'master')
    if sp_master_count != 1:
        raise ex.InvalidComponentCountException('master', 1, sp_master_count)
