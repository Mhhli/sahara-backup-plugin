...
from sahara.plugins.sandbox.hadoop2 import oozie_helper
    ...
    with context.ThreadGroup() as tg:
        for instance in instances:
            with context.set_current_instance_id(instance.instance_id):
                ...
                tg.spawn('sandbox-start-processes-%s' % instance.instance_name,
...
def start_oozie_process(pctx, instance, backup=None):
    with context.set_current_instance_id(instance.instance_id):
        with instance.remote() as r:
            if c_helper.is_mysql_enabled(pctx, instance.cluster):
                ...
                if backup is None:
                    ...
                    sql_script = files.get_file_text(
                        'plugins/sandbox/hadoop2/resources/create_oozie_db.sql')
...
# newly added method for starting spark
@cpo.event_wrapper(
    True, step=pu.start_process_event_message("Spark"))
def start_spark(master):
    sp_home = c_helper.get_spark_home(master.cluster)
    with context.set_current_instance_id(master.instance_id):
        with master.remote() as r:
            r.execute_command('sudo su - -c "bash %s" hadoop' % os.path.join(
                sp_home, 'sbin/start-all.sh'))
...
def _hive_copy_shared_conf(remote, dest, backup=None):
    ...
    if backup is None:
        ...
        remote.execute_command(
            "sudo su - -c 'hadoop dfsadmin -safemode leave && "
            "hadoop fs -put -f /opt/hive/conf/hive-site.xml "
            "%s' hadoop" % dest)
...
def _hive_create_db(remote):
    ...
    remote.execute_command("export HADOOP_HOME=/opt/hadoop; export PATH=$PATH:$HADOOP_HOME/bin:$HADOOP_HOME/sbin;"
...
def _hive_hiveserver2_start(remote):
    LOG.debug("Starting Hive Hiveserver2 Server")
    remote.execute_command("sudo su - -c 'nohup /opt/hive/bin/hive"
                           " --service hiveserver2 > /dev/null &' hadoop")
    ...
    with context.set_current_instance_id(instance.instance_id):
        with instance.remote() as r:
            ...
            _hive_copy_shared_conf(
                r, edp.get_hive_shared_conf_path('hadoop'), backup)
...
def config_env(instance, data):
    if instance:
        with instance.remote() as r:
            r.append_to_file('/etc/bash.bashrc', data, run_as_root=True)
            r.execute_command('source /etc/bash.bashrc', run_as_root=True)
